...
static void share_cb(lv_event_t* e) {
    ...
    // Send waypoint via LXMF to first available peer
    bool sent = false;
    const crypto::Identity* id = ui_get_identity();
    const uint8_t* lxmf_dest = ui_get_lxmf_dest();
    if (id && lxmf_dest) {
        const net::Peer* peer = net::peer_first();
        if (peer) {
            sent = msg::waypoint_send(*id, lxmf_dest, peer->dest_hash, name, "");
        }
    }

    // Always add to local waypoint list as confirmation
    ui_on_waypoint_received("Me", name,
                            hal::gps_latitude(), hal::gps_longitude(),
                            hal::gps_altitude(),
                            category_values[selected_category], 0);

    Serial.printf("[UI] %s waypoint: %s (%.4f, %.4f) cat=%d\n",
                  sent ? "Sent" : "Shared", name,
                  hal::gps_latitude(), hal::gps_longitude(), selected_category);
