...
// Phase 4a.5: Added lxmf_dest_hash for message routing
...
struct Peer {
    uint8_t dest_hash[DEST_HASH_SIZE];         // Announce destination (traildrop.waypoint)
    uint8_t lxmf_dest_hash[DEST_HASH_SIZE];    // Message destination (lxmf.delivery)
    ...
    char app_data[DISPLAY_NAME_MAX];            // Display name
...
// lxmf_dest may be nullptr (zeroed in that case).
